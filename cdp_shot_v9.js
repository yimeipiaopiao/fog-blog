// v9 视觉验证：复现 "localStorage 旧值覆盖 server 新值" 的隐蔽 bug
// 场景：DB=sea（已通过 API 设置），但浏览器 localStorage 'wb-palette'='amber'（陈旧值）
// 已登录 admin 访问 /admin/pages
// 期望（修复后）：data-palette=sea（server 权威）
// 实际（修复前）：data-palette=amber（localStorage 覆盖回去）
...
const TARGETS = [
  { p: "A:/admin/pages", name: "v9_repro_stale_localStorage_amber",
    admin: true, splash: false, fullPage: true, stalePalette: "amber" },
];
