...
| 前台首页（海盐蓝 浅色） | 前台首页（深色） | 后台仪表板 |
| --- | --- | --- |
| ![前台首页](screenshots/home-sea.png) | ![前台深色模式](screenshots/home-dark.png) | ![后台仪表板](screenshots/admin-dashboard.png) |
| 毛玻璃 + 5 套配色 + 侧栏每日鸡汤 / 人生倒计时 / 免费天气 / 标签云 / 热门 | 深色模式（手动 / 定时 / 跟随系统自动切换） | 实时统计、最近文章、最新评论待审核 |

| 文章列表 / 后台 | 站点设置 | SSL 证书（宝塔式后台上传） |
| --- | --- | --- |
| ![站点设置](screenshots/admin-settings.png) | ![文章详情](screenshots/article.png) | 后台「SSL 证书」粘贴 PEM 即写盘 + nginx -t + reload，无需登录服务器 |
...
| 互动评论 | 自建评论 + 审核、游客免注册评论（**按 IP 自动派生稳定昵称**）、浏览统计（session 去重）、点赞、留言板、文章级评论开关、**中央确认弹窗**（自定义毛玻璃，覆盖所有删除/危险操作）|
...
| 系统管理 | 仪表板、站点设置、操作日志（自动清理）、数据备份（下载 DB + JSON 导出）、**SSL 证书后台上传**（宝塔式：粘贴 PEM → 自动解析校验 → 写 `/etc/nginx/ssl/<域>/` + nginx -t + reload，事务式失败回滚 + 过期前 30 天提醒） |
...
后台「文章管理」筛选条右侧可切换：发布时间 ↓ / 发布时间 ↑ / 热度 🔥（点赞×5 + 评论×10 + 阅读），可与「全部 / 已发布 / 草稿 / 定时」组合使用。写文章时输入会每 1.2 秒自动存到浏览器本地草稿（localStorage），中途关页面 / 刷新 / 死机都不丢；下次进入编辑器顶部出现琥珀色提示条，点「恢复草稿」一键还原，「丢弃」清除。点「保存」提交成功会自动清空本地草稿。
