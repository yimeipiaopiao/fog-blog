// v11 视觉验证：文案改名后的前台 sidebar 实际效果
// 截图目标：跳过 splash 后截前台首页（fullPage），看 "每日鸡汤" + "人生倒计时"
...
async function main() {
  ...
  for (const t of TARGETS) {
    ...
    // 4) splash 跳过（v11 强制移除，避免 splash DOM 挡住 sidebar）
    await call("Runtime.evaluate", {
      expression: `
        try {
          // 标记 24h 内已看过，splash.js 不会渲染
          localStorage.setItem('wb-splash-seen', String(Date.now()));
          // 兜底：直接移除已存在的 splash DOM
